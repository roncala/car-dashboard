...
function validate_phone_local(?string $v): ?string {
  ...
  if (!preg_match('/^[0-9+\-\s().]{7,25}$/', $v)) bad_request('Invalid phone format.');
  return $v;
}
function validate_zip_local(?string $v): ?string {
  $v = str_clean($v, 15);
  if ($v === null) return null;
  if (!preg_match('/^\d{5}(-\d{4})?$/', $v)) bad_request('Invalid ZIP code format.');
  return $v;
}
function validate_state_local(?string $v): ?string {
  $v = str_clean($v, 40);
  if ($v === null) return null;
  if (!preg_match('/^[A-Za-z]{2}$/', $v)) bad_request('State must be 2 letters (e.g., NJ).');
  return strtoupper($v);
}
function validate_country_local(?string $v): string {
  $v = str_clean($v, 60);
  if ($v === null) return 'USA';
  if (!preg_match('/^[A-Za-z][A-Za-z \-]{1,59}$/', $v)) bad_request('Invalid country.');
  return $v;
}
